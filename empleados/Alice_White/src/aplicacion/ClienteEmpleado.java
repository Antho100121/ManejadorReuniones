...
public class ClienteEmpleado {
    ...
    public static void main(String[] args) {
        String nombreEmpleado = "Alice_White";
        Scanner scanner;
        try {
    ...
    private static Reunion crearReunion(Scanner scanner, String organizador) {
        ...
        return new Reunion(tema, organizador, Arrays.asList(invitados), lugar, inicio, fin);
    }
        ...
        for (int intento = 1; intento <= MAX_INTENTOS; intento++) {
            try (Socket socket = new Socket("central-server", 9090);
                ...
                if (intento < MAX_INTENTOS) {
                    try {
                        Thread.sleep(2000); // esperar 1 segundo antes de reintentar
    ...
    private static void esperarServidorDisponible() {
        ...
        while (intentos > 0) {
            try (Socket s = new Socket("central-server", 9090)) {
                // Si la conexión tiene éxito, el servidor está listo
                return;
            } catch (IOException e) {
